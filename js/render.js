...
(function (g) {
  ...
  function overview() {
    ...
    return `
      ...
      <div class="kpi-grid">
        ...
        <article class="kpi"><div class="n">${k.closedCapaYtd}</div><div class="l">CAPAs closed YTD</div><p>${k.complaintsYtd} complaints logged YTD.</p></article>
      </div>
      <div class="split">
        <section class="card">
          <h2>Overdue CAPAs</h2>
  ...
  function ncs() {
    ...
    return `<section class="hero"><h1>Nonconformances</h1><p class="lede">Incoming, in-process, audit, and complaint-driven NCs. Click a row.</p>
  ...
  function capas() {
    ...
    return `<section class="hero"><h1>CAPA</h1><p class="lede">8D-style records. Only QA Manager can close.</p></section>
    ...
    return `<section class="hero"><p class="kicker">Nonconformance</p><h1>${esc(n.id)}</h1>${pill(n.status)} ${pill(n.severity)}</section>
      <section class="card prose">
        ...
        <dl>
          ...
          <dt>Links</dt><dd>CAPA ${esc(n.capa_id || "—")} · SCAR ${esc(n.scar_id || "—")} · Complaint ${esc(n.complaint_id || "—")}</dd>
        </dl>
      </section>`;
  }
  ...
  function detailCapa(id) {
    ...
    return `<section class="hero"><p class="kicker">CAPA</p><h1>${esc(c.id)}</h1>${pill(c.status)}
      ...
      ${g.HelixStore.canCloseCapa() && c.status !== "closed" ? '<button class="btn" data-action="close-capa" data-id="'+esc(c.id)+'">Close CAPA (QA Manager)</button>' : ""}
      ${!g.HelixStore.canCloseCapa() && c.status !== "closed" ? '<p class="hint">Switch role to QA Manager to close.</p>' : ""}
      </section>
      <section class="card prose">
  ...
  function valView() {
    ...
    let body = `<section class="hero"><h1>Computer system validation</h1>
      <p class="lede">IQ checks the install. OQ scripts the desk. PQ runs a real complaint→SCAR path. Evidence is pass/fail against this app, not a screenshot of a blank template.</p>
    ...
    <section class="card"><h2>Requirements traceability</h2>
    ...
    if (r) {
      ...
      body += `<section class="card report" id="val-report">
        ...
        <p>${esc(r.executor)} · ${esc(r.finished)} · app ${esc(r.version)} · seed ${esc(r.checksum)} · ${r.totals.pass}/${r.totals.n} pass</p>
        ${block("IQ Installation", r.iq)}
        ${block("OQ Operational", r.oq)}
        ${block("PQ Performance / intended use", r.pq)}
        ${r.failed.length ? "<p class='hint'>Failed: " + esc(r.failed.join(", ")) + "</p>" : "<p>No deviations. Protocol complete.</p>"}
      </section>`;
    }
    return body;
  }
  ...
  function reports() {
    ...
    return `<section class="hero"><h1>Management review pack</h1>
      ...
      <section class="card prose" id="mgmt-review">
        <p class="kicker">${esc(d.meta.company.name)} · ${esc(d.meta.today)} · SYNTHETIC</p>
        <h2>Quality summary</h2>
        <ul>
          <li><strong>${k.overdueCapa}</strong> overdue CAPAs (action this week)</li>
          <li><strong>${k.openNc}</strong> open NCs · <strong>${k.criticalOpen}</strong> critical still open</li>
          <li><strong>${k.openScar}</strong> open/late SCARs · <strong>${k.expiringCerts}</strong> supplier certs inside 90 days (${k.expiredCerts} expired)</li>
